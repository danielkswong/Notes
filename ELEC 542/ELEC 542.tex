...
\begin{center}
\begin{tikzpicture}
	\draw (-3,0) coordinate (A) node[above right,yshift=0.1cm] {\small C} -- ++(0:2) coordinate (B) node[above left,yshift=0.1cm] {\small N};
	...
	\draw (A) -- ++(240:1) node[below left] {\small H};
	\draw (B) -- ++(300:1) node[below right] {\small H};
	\draw (C) -- ++(0:1) node[right] {\small H};
	\draw (D) -- ++(60:1) node[above right] {\small H};
	\draw (E) -- ++(120:1) node[above left] {\small H};
	\draw (F) -- ++(180:1) node[left] {\small H};
	...
	\draw[red,<-,shorten <=2mm] (B) -- ++(1,0) node[right,align=center,text width=4cm] {contributes 1 electron to the $\pi$ orbital};
	\node at (-2,-2) {pyridine (protonated)};

	\draw (5,-7) coordinate (G) node[above,yshift=0.1cm] {\small N} -- ++(36:2) coordinate (H) node[left,xshift=-0.1cm] {\small C};
	\draw[double distance=3pt] (H) -- ++(108:2) coordinate (I) node[below left,yshift=-0.1cm] {\small C};
	\draw (I) -- ++(180:2) coordinate (J) node[below right,yshift=-0.1cm] {\small C};
	\draw[double distance=3pt] (J) -- ++(252:2) coordinate (K) node[right,xshift=0.1cm] {\small C};
	\draw (K) -- (G);
	\draw (G) -- ++(270:1) node[below] {\small H};
	\draw (H) -- ++(348:1) node[right] {\small H};
	\draw (I) -- ++(54:1) node[above right] {\small H};
	\draw (J) -- ++(126:1) node[above left] {\small H};
	\draw (K) -- ++(198:1) node[left] {\small H};
	\draw[white,fill=black,line width=2pt] (G) circle (3pt);
	\draw[white,fill=black,line width=2pt] (H) circle (3pt);
	\draw[white,fill=black,line width=2pt] (I) circle (3pt);
	\draw[white,fill=black,line width=2pt] (J) circle (3pt);
	\draw[white,fill=black,line width=2pt] (K) circle (3pt);
	\draw[red,<-,shorten <=2mm] (G) -- ++(-1,0) node[left,align=center,text width=4cm] {contributes 2 electron to the $\pi$ orbital};
	\node at (5,-9) {pyrrole};
\end{tikzpicture}
\end{center}
The H\"{u}ckel parameter would be different for the nitrogen atoms in the two cases above. The effective Hamiltonian is written as follows:
\begin{itemize}
\item Diagonal elements for carbon atoms are $\alpha_{\trm{C}}$
\item Off-diagonal elements for carbon-carbon are $\beta_{\trm{CC}}$
\item For other atoms: $\begin{cases}
\alpha_{x}=\alpha_{\trm{c}}+h_{x}\beta_{\trm{CC}}\\
\beta_{xy}=k_{xy}\beta_{\trm{CC}}
\end{cases}$
\end{itemize}
For example, for $N$ contributing one $\pi$ electron yields $h_{x}=0.5$ and $k_{xy}=0.84$ (for connection to C). For example, in pyridine
\begin{center}
\begin{tikzpicture}
	\draw (-3,0) coordinate (A) node[above right,yshift=0.2cm] {\small6} -- ++(0:2) coordinate (B) node[above left,yshift=0.2cm] {\small1};
	\draw[double distance=3pt] (B) -- ++(60:2) coordinate (C) node[left,xshift=-0.2cm] {\small2};
	\draw (C) -- ++(120:2) coordinate (D) node[below left,yshift=-0.2cm] {\small3};
	\draw[double distance=3pt] (D) -- ++(180:2) coordinate (E) node[below right,yshift=-0.2cm] {\small4};
	\draw (E) -- ++(240:2) coordinate (F) node[right,xshift=0.2cm] {\small5};
	\draw[double distance=3pt] (F) -- (A);
	\draw (A) -- ++(240:1) node[below left] {\small H};
	\draw (B) -- ++(300:1) node[below right] {\small H};
	\draw (C) -- ++(0:1) node[right] {\small H};
	...
	\draw (E) -- ++(120:1) node[above left] {\small H};
	\draw (F) -- ++(180:1) node[left] {\small H};
	\node[fill=white] at (A) {\small C};
	\node[fill=white] at (B) {\small N};
	\node[fill=white] at (C) {\small C};
	\node[fill=white] at (D) {\small C};
	\node[fill=white] at (E) {\small C};
	\node[fill=white] at (F) {\small C};
\end{tikzpicture}
\end{center}
\begin{equation}
\vb{F}=\mqty[\alpha_{\trm{C}}+0.5\beta_{\trm{CC}} & 0.8\beta_{\trm{CC}} & 0 & 0 & 0 & 0.8\beta_{\trm{CC}}\\
0.8\beta_{\trm{CC}} & \alpha_{\trm{C}} & \beta_{\trm{CC}} & 0 & 0 & 0\\
0 & \beta_{\trm{CC}} & \alpha_{\trm{C}} & \beta_{\trm{CC}} & 0 & 0\\
0 & 0 & \beta_{\trm{CC}} & \alpha_{\trm{C}} & \beta_{\trm{CC}} & 0\\
0 & 0 & 0 & \beta_{\trm{CC}} & \alpha_{\trm{C}} & \beta_{\trm{CC}}\\
0.8\beta_{\trm{CC}} & 0 & 0 & 0 & \beta_{\trm{CC}} & \alpha_{\trm{C}}]
\end{equation}
$\vb{S}$ is taken to be unity ($\id$). There is also no iterative process as was done in Hartree-Fock.\\
...
\end{equation}
\begin{tikzpicture}[remember picture,overlay]
	\draw[red,<-,shorten <=2mm] (aa.south) -- ++(0.5,-0.5) node[right] {\small separation between two centres};
	\draw[red,<-,shorten <=1mm] (bb.east) -- ++(0.5,0) node[right] {\small determined using spectral data};
\end{tikzpicture}

and
\begin{equation}
F_{ij}=\beta_{ij}-\frac{1}{2}P_{ij}\gamma_{ij}
\end{equation}
One can also include \ul{neglect of diatomic differential overlap (NDDO)}, where the one-electron integrals that involve three centres are neglected. In other words, for $[i|h|j]$, terms that come from nuclei that do not host $i$ or $j$ are neglected.\\

Consider \ul{intermediate neglect of differential overlap (INDO)}. Compared to ZDO, $[ij|k\ell]$ vanishes if $i\neq j$ or $k\neq\ell$ (i.e. must be the same basis function), unless if $i$, $j$, $k$, and $\ell$ all lie on the same centre. For \ul{complete neglect of} \ul{differential overlap (CNDO)}, $[ij|k\ell]=0$ if $i\neq j$ or $k\neq\ell$ in all cases.\\

\newpage
For complete neglect of differential overlap, some parameterizations include:
\begin{itemize}
\item All remaining two-electron integrals involving atoms $A$ and $B$ are assumed to be equal (regardless of basis functions involved)
\begin{equation}
F_{ii}={\color{red}\underbrace{\color{black}U_{ii}}_{\mathrlap{=\int\phi_{i}^{*}(\vec{r}_{1})\qty(-\frac{1}{2}\nabla^{2}-\frac{Z_{A}}{R_{A}})\phi_{i}(\vec{r}_{1})\dd{\vec{r}_{1}}}}}+\qty(P_{AA}-\frac{1}{2}P_{ii})\gamma_{AA}+\sum_{B\neq A}(P_{BB}\gamma_{AB}-{\color{red}\underbrace{\color{black}V_{AB}}_{\mathclap{\substack{\trm{contributions from}\\\trm{other nuclei}}}}})
\end{equation}
with $P_{AA}$ being the sum of diagonals for all the basis functions on atom $A$. Additionally
\begin{equation}
F_{ij}={\color{red}\underbrace{\color{black}\beta_{AB}}_{\mathclap{\substack{\trm{parameter chosen from comparison}\\\trm{with ab-initio calculations}}}}}S_{ij}-\frac{1}{2}P_{ij}\gamma_{AB}
\end{equation}
\item Alternatively, $U_{ii}$ is calculated as
\begin{equation}
U_{ii}=-\frac{1}{2}\qty(\tikzmarknode{aa}{I_{i}}+\tikzmarknode{bb}{E_{i}})-\qty(Z_{A}-\frac{1}{2})\gamma_{AA}\qq{,}V_{AB}=-Z_{B}\gamma_{AB}
\end{equation}
\end{itemize}
\begin{tikzpicture}[remember picture,overlay]
	\draw[red,<-,shorten <=1mm] (aa.south) -- ++(-0.3,-0.5) node[below left,xshift=0.3cm] {\footnotesize ionization energy};
	\draw[red,<-,shorten <=1mm] (bb.south) -- ++(0.4,-0.5) node[below right,xshift=-0.3cm] {\footnotesize electron affinity};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.7]
	...
	\draw[ultra thick,->] (0,0) -- (4,0) node[right]{$y$};
	\draw[ultra thick,->] (0,0) -- (-1,-1) node[below left]{$x$};
	\draw[->] (0,0) -- node[below,xshift=0.2cm,yshift=0.1cm]{$\vec{r}_{i}$} (3,1.5) node[right]{$m_{i}$};
\end{tikzpicture}
\end{center}
Consider some basic force fields. One example is Hooke's law.
\begin{center}
\begin{tikzpicture}
	\draw[decoration={aspect=0.3,segment length=3mm,amplitude=3mm,coil},decorate] (0,0) -- (3,0);
	\filldraw[black] (-0.5,0) circle (2pt);
	\filldraw[black] (3.5,0) circle (2pt);
	\draw (-0.5,0) -- (0,0);
	\draw (3,0) -- (3.5,0);
	\draw[->] (0,-1) -- node[below] {$\vec{r}$} (3,-1);
	\draw[->] (5,-1) -- (9.5,-1) node[right] {$r$};
	\draw[->] (5.5,-1.5) -- (5.5,1.5) node[above] {$U$};
	\draw (7.5,-0.9) -- (7.5,-1.1) node[below] {$r_{0}$};
	\draw[domain=6.1:8.9,samples=200,<->] plot (\x, {(\x-7.5)^2-0.5});
	\node[right] at (9.5,0.5) {$\vec{F}=-\vec{\nabla}U$};
\end{tikzpicture}
\end{center}
...
One improvement to this force field is the Morse potential.
\begin{center}
\begin{tikzpicture}
	\draw[->] (-0.5,0) -- (6,0) node[right] {$r$};
	\draw[->] (0,-0.5) -- (0,4) node[above] {$U$};
	\draw[dashed] (-0.1,1) node[left] {$D_{e}$} -- (6,1);
	\draw[domain=0.2:5.8,samples=200,<->] plot (\x, {(1-exp(-0.6*(\x-2)))^2});
	\draw (2,0.1) -- (2,-0.1) node[below] {$r_{0}$};
	\node[right] at (7,2) {$U=D_{e}\qty[1-\exp\big(\beta(r-r_{0})\big)]^{2}$};
\end{tikzpicture}
\end{center}
A more general form for the force field looks like
\begin{equation}
\begin{aligned}
U_{\trm{tot}}&=\sum_{A,B}U_{AB}+\sum_{\mathclap{A,B,C}}U_{ABC}+\sum_{\mathclap{A,B,C,D}}U_{ABCD,\trm{dih}}\\
&\quad+\sum_{\mathclap{A,B,C,D}}U_{ABCD,\trm{out of plane}}+\sum U_{\trm{non-bonded}}+\sum U_{\trm{Coulomb}}
\end{aligned}
\end{equation}
\begin{center}
\begin{tikzpicture}
	\draw (0,0) -- node[below] {\footnotesize bond length} (1,0);
	\filldraw[black] (0,0) node[above] {$A$} circle (2pt);
	\filldraw[black] (1,0) node[above] {$B$} circle (2pt);
	\draw (3,0) -- (4,0) node[below] {\footnotesize bond angle} -- (4.7,0.7);
	\filldraw[black] (3,0) node[above] {$A$} circle (2pt);
	\filldraw[black] (4,0) node[above] {$B$} circle (2pt);
	\filldraw[black] (4.7,0.7) node[above] {$C$} circle (2pt);
	\filldraw[draw=black,fill=gray,opacity=0.2] (7,0.8,0) -- (7,0.8,2) -- (9,0.8,2) -- (9,0.8,0) -- cycle;
	\filldraw[draw=red,fill=red,opacity=0.2] (7,0.5,0.5) -- (7,1.8,0.5) -- (9,1.8,0.5) -- (9,0.5,0.5) -- cycle;
	\filldraw[black] (7.3,0.8,1.5) node[above left,xshift=-0.3cm] {$A$} circle (2pt);
	\filldraw[black] (7.3,0.8,0.5) node[above left,xshift=-0.3cm] {$B$} circle (2pt);
	\filldraw[black] (8.7,0.8,0.5) node[above right,xshift=0.3cm] {$C$} circle (2pt);
	\filldraw[black] (8.7,1.5,0.5) node[above right,xshift=0.3cm] {$D$} circle (2pt);
	\draw (7.3,0.8,1.5) -- (7.3,0.8,0.5) --(8.7,0.8,0.5) -- (8.7,1.5,0.5);
	\node[below] at (7.5,0) {\footnotesize dihedral angle};
	\filldraw[draw=black,fill=gray,opacity=0.2] (11,0.8,0) -- (11,0.8,2) -- (13,0.8,2) -- (13,0.8,0) -- cycle;
	\filldraw[black] (11.3,0.8,1.5) node[above left,xshift=-0.3cm] {$A$} circle (2pt);
	\filldraw[black] (11.3,0.8,0.5) node[above left,xshift=-0.3cm] {$C$} circle (2pt);
	\filldraw[black] (12.7,0.8,1) node[right,xshift=0.3cm] {$B$} circle (2pt);
	\filldraw[black] (12,1.4,1) node[above] {$D$} circle (2pt);
	\draw (11.3,0.8,1.5) -- (12.7,0.8,1) -- (11.3,0.8,0.5);
	\draw (12.7,0.8,1) -- (12,1.4,1);
	\node[below] at (11.5,0) {\footnotesize out-of-plane angle};
\end{tikzpicture}
\end{center}
One example is
\begin{equation}
...
There are different time propagations. Consider the \ul{leapfrog algorithm}.
\begin{equation*}
\left\{
\begin{aligned}
v_{i}\qty(t+\frac{\Delta t}{2})&=v_{i}(t)+\dv{v_{i}}{t}\Bigr|_{t}\qty(\frac{\Delta t}{2})+\frac{1}{2}\dv[2]{v_{i}}{t}\Bigr|_{t}\qty(\frac{\Delta t}{2})^{2}+\ldots\\
v_{i}\qty(t-\frac{\Delta t}{2})&=v_{i}(t)+\dv{v_{i}}{t}\Bigr|_{t}\qty(-\frac{\Delta t}{2})+\frac{1}{2}\dv[2]{v_{i}}{t}\Bigr|_{t}\qty(-\frac{\Delta t}{2})^{2}+\ldots
\end{aligned}\right.
\end{equation*}
\begin{equation}
\implies v_{i}\qty(t+\frac{\Delta t}{2})=v_{i}\qty(t-\frac{\Delta t}{2})+{\color{red}\underbrace{\color{black}\dv{v_{i}}{t}\Bigr|_{t}}_{\mathclap{a_{i}(t)=\frac{F_{i}(t)}{m}}}}\qty(\Delta t)\qq{\color{red}(neglecting higher orders)}
\end{equation}
Similarly
\begin{equation}
r_{i}(t+\Delta t)=r_{i}(t)+{\color{red}\underbrace{\color{black}\dv{r_{i}}{t}\Bigr|_{t+\frac{\Delta t}{2}}}_{v_{i}\qty(t+\frac{\Delta t}{2})}}\qty(\Delta t)
\end{equation}
\begin{center}
\begin{tikzpicture}
	\draw[->] (-0.5,0) -- (8,0) node[right] {$t$};
	\draw (0,-0.1) -- (0,0.1) node[above] {$v_{i}\qty(t-\frac{\Delta t}{2})$};
	\draw (3,-0.1) -- (3,0.1) node[above] {$v_{i}\qty(t+\frac{\Delta t}{2})$};
	\draw (6,-0.1) -- (6,0.1);
	\filldraw[black] (1.5,0) node[below] {$r_{i}(t)$} circle (2pt);
	\filldraw[black] (4.5,0) node[below] {$r_{i}(t+\Delta t)$} circle (2pt);
	\filldraw[black] (7.5,0) circle (2pt);
	\draw[->,shorten <=1mm,shorten >=1mm] (0,1) to[bend left] (3,1);
	\draw[->,shorten <=1mm,shorten >=1mm] (3,1) to[bend left] (6,1);
	\draw[->,shorten <=1mm,shorten >=1mm] (1.5,-1) to[bend right] (4.5,-1);
	\draw[->,shorten <=1mm,shorten >=1mm] (4.5,-1) to[bend right] (7.5,-1);
\end{tikzpicture}
\end{center}
Consider the \ul{Verlet algorithm}.
\begin{equation*}
\left\{
\begin{aligned}
r_{i}\qty(t+\Delta t)&=r_{i}(t)+\dv{r_{i}}{t}\Bigr|_{t}\qty(\Delta t)+\frac{1}{2}\dv[2]{r_{i}}{t}\Bigr|_{t}\qty(\Delta t)^{2}+\ldots\\
r_{i}\qty(t-\Delta t)&=r_{i}(t)+\dv{r_{i}}{t}\Bigr|_{t}\qty(-\Delta t)+\frac{1}{2}\dv[2]{r_{i}}{t}\Bigr|_{t}\qty(-\Delta t)^{2}+\ldots
\end{aligned}\right.
\end{equation*}
\begin{equation}
\implies r_{i}\qty(t+\Delta t)=2r_{i}(t)-r_{i}(t-\Delta t)+{\color{red}\underbrace{\color{black}\dv[2]{r_{i}}{t}\Bigr|_{t}}_{\mathclap{a_{i}(t)=\frac{F_{i}(t)}{m}}}}\qty(\Delta t)^{2}\qq{\color{red}(neglecting higher orders)}
\end{equation}
If the velocities of the atoms are desired
\begin{equation}
v_{i}(t)=\frac{r_{i}(t+\Delta t)-r_{i}(t-\Delta t)}{2\Delta t}
\end{equation}
Consider the \ul{velocity Verlet algorithm}.
\begin{equation}
\begin{aligned}
r_{i}(t+\Delta t)&=r_{i}(t)+{\color{red}\underbrace{\color{black}\dv{r_{i}}{t}\Bigr|_{t}}_{v_{i}(t)}}+\frac{1}{2}{\color{red}\underbrace{\color{black}\dv[2]{r_{i}}{t}\Bigr|_{t}}_{a_{i}(t)}}(\Delta t)^{2}\\
v_{i}(t+\Delta t)&=v_{i}(t)+\frac{a_{i}(t)+a_{i}(t+\Delta t)}{2}\Delta t
\end{aligned}
\end{equation}

\subsection{Physics of Molecular Dynamics Considerations}
There are a number of considerations one should account for before performing a molecular dynamics simulation
\begin{itemize}[noitemsep]
\item Choice of force field (interatomic potential)
\item Time propagation algorithm
\item Time step (should be smaller than or comparable to the time scale of atomic motion, $\sim$ fs order)
\item Number of time steps required
\item Boundary conditions (including periodicity provided they are larger than the length scales of importance)
\item Key features/parameters that describe the system (fixed or changing) which may include particle number, volume, pressure, temperature
\begin{itemize}[nolistsep]
\item May require different statistical ensembles such as $nVE$ (microcanonical), $nVT$ (canonical), or $\mu VE$ (grand canonical)
\end{itemize}
\end{itemize}
...
\begin{tikzpicture}
	...
	\draw[->] (0,0,0) -- (0,0,2.5) node[below left] {$z$};
	\filldraw[draw=black,fill opacity=0.2] (0,0,2) -- (2,0,2) -- (2,2,2) -- (0,2,2) -- cycle;
	\node[above left] at (0,0,2) {$L_{x}$};
	\draw (2,0,0) node[below right] {$L_{x}$} -- (2,2,0) -- (2,2,2) -- (2,0,2) -- cycle;
	\draw (0,2,0) node[above left] {$L_{y}$} -- (0,2,2) -- (2,2,2) -- (2,2,0) -- cycle;
\end{tikzpicture}
\end{center}
\begin{equation}
...
One simple thermostat is \ul{velocity scaling}.
\begin{equation}
v_{i}^{\trm{new}}(t)=\sqrt{\frac{T_{\trm{MD}}}{T(t)}}v_{i}^{\trm{old}}(t)
\end{equation}
Another example is the \ul{Berendesen thermostat}.
\begin{equation}
\dv{v_{i}}{t}=\frac{F_{i}}{m_{i}}+\frac{1}{2\tau}\qty(\frac{T_{\trm{MD}}}{T(t)}-1)v_{i}
\end{equation}
There is also the \ul{Andersen thermostat}. Knowing that particles should follow a Maxwell-Boltzmann distribution, velocities of a prescribed number of particles are drawn from a Maxwell-Boltzmann distribution corresponding to the desired temperature and the resulting particles are allowed to interact with the system.\\

Consider \ul{Langevin dynamics}.
\begin{equation}
m_{i}\dv{v_{i}}{t}=F_{i}(t)-m_{i}\Gamma v_{i}(t)+A_{i}(t)
\end{equation}
where $\Gamma$ is the friction coefficient and $A_{i}(t)$ is random noise that depends on temperature.\\

One famous example is the \ul{Nos\'{e}-Hoover thermostat}. This uses the velocity Verlet algorithm.
\begin{equation}
\begin{aligned}
r_{i}(t+\Delta t)&=r_{i}(t)+v_{i}(t)\Delta t+\frac{1}{2}a_{i}(t)\Delta t^{2}\\
v_{i}(t+\Delta t)&=v_{i}(t)+\frac{a_{i}(t)+a_{i}(t+\Delta t)}{2}\Delta t\\
&=v_{i}\qty(t+\frac{\Delta t}{2})+\frac{a_{i}(t+\Delta t)}{2}\Delta t
\end{aligned}
\end{equation}
The thermostat follows
\begin{equation}
m_{i}a_{i}(t)=F_{i}(t)-\mu m_{i}v_{i}(t)
\end{equation}
where $\mu$ evolves following
\begin{equation}
\dv{\mu}{t}=\frac{1}{\tikzmarknode{aa}{Q}}\Bigg[\,{\color{red}\underbrace{\color{black}\sum_{i=1}^{N}\frac{1}{2}m_{i}v_{i}^{2}(t)-\frac{3N+\tikzmarknode{bb}{1}}{2}k_{B}T_{\trm{MD}}}_{\substack{\trm{deviation between actual}\\ \trm{and expected kinetic energies}}}}\,\Bigg]
\end{equation}
\begin{tikzpicture}[remember picture,overlay]
	\draw[red,<-,shorten <=1mm] (aa.south) -- ++ (0,-0.5) node[below] {\footnotesize weight};
	\draw[red,<-,shorten <=1mm] (bb.north) -- ++ (0.5,0.5) node[right] {\footnotesize extra degree of freedom from $\mu$};
\end{tikzpicture}

Thus
\begin{equation}
\begin{aligned}
\mu\qty(t+\frac{\Delta t}{2})&=\mu(t)+\frac{1}{Q}\qty[\sum_{i=1}^{N}\frac{1}{2}m_{i}v_{i}^{2}(t)-\frac{3N+1}{2}k_{B}T_{\trm{MD}}]\frac{\Delta t}{2}\\
\mu\qty(t+\Delta t)&=\mu(t+\frac{\Delta t}{2})+\frac{1}{Q}\qty[\sum_{i=1}^{N}\frac{1}{2}m_{i}v_{i}^{2}(t)-\frac{3N+1}{2}k_{B}T_{\trm{MD}}]\frac{\Delta t}{2}
\end{aligned}
\end{equation}
Following the velocity Verlet equation and the equation describing the thermostat
\begin{equation}
\begin{aligned}
v_{i}\qty(t+\Delta t)&=v_{i}\qty(t+\frac{\Delta t}{2})+\frac{1}{2m_{i}}\qty[F_{i}\qty(t+\Delta t)-\mu(t+\Delta t)m_{i}v_{i}(t+\Delta t)]\Delta t\\
&= \frac{v_{i}\qty(t+\frac{\Delta t}{2})+\qty(\frac{\Delta t}{2m_{i}})F_{i}(t+\Delta t)}{1+\frac{\Delta t}{2}\mu(t+\Delta t)}
\end{aligned}
\end{equation}
Moving to barostats, one simple barostat is \ul{volume scaling}. Recall the virial theorem relating pressure and volume. If the pressure differs from the desired value as the system evolves, the volume can be adjusted such that the pressure obtains the value required.\\

Another barostat is the \ul{Berendsen barostat}. The coordinates are scaled by $\eta^{\frac{1}{3}}$, such that the volume is scaled by $\eta$, where
\begin{equation}
\eta(t)=1-\frac{\Delta t}{\tau_{P}}\gamma\qty(P_{\trm{MD}}-P(t))
\end{equation}
with $\tau_{P}$ gives the relaxation time and $\gamma$ gives the compressibility of the system.\\

There is also the \ul{Andersen barostat}, which follows
\begin{equation}
\begin{aligned}
\dv[2][{V}{t}&=\frac{P(t)-P_{\trm{MD}}}{Q}\qq{,}\begin{cases}
r_{i}=V^{\frac{1}{3}}s_{i}\\
v_{i}=V^{\frac{1}{3}}\dv{s_{i}}{t}
\end{cases}\\
\implies \dv[2]{s_{i}}{t}&=\frac{F_{i}}{m_{i}V^{\frac{1}{3}}}-\frac{2}{3}\frac{\dv{V}{t}}{V}\qty(\dv{s_{i}}{t})
\end{aligned}
\end{equation}
where $s_{i}$ is an auxiliary variable.\\

Consider finally the \ul{Parinello-Rahman barostat}. It is an extension of the Andersen barostat, but allows the shape of the simulation box to change.
