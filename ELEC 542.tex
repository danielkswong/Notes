...
This Fock operator itself will depend on $\{\Chi_{i}(\vec{x}_{1})\}$ that needs to be solved in a self-consistent manner. It will still contain a kinetic term for the electrons, electron-nuclei attraction, but it will transform the electron-electron repulsion into some kind of average electron-electron interaction.
...
\end{equation}
\aside{Proof}{Assume that $\{\phi_{\alpha}(\vec{x}_{1},\cdot,\vec{x}_{N})\}$ is the actual exact solution of the $N$-electron problem.
\begin{equation}
